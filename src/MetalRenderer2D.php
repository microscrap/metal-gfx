...
/**
 * Metal DrawingAPI — writes into a borrowed tubes Framebuffer (typically
 * {@see MetalHandledFramebuffer}). Whole-surface {@see fill()} uses the buffer's
 * Metal texture clear when bound to Metal; primitives are CPU-side into the
...
class MetalRenderer2D extends Renderer2D implements ProvisionsHeadlessFramebuffer
{
    ...
    {
        // MetalHandledFramebuffer::fill → mtl_texture_clear (GPU).
